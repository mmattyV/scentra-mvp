...
export default function Header() {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (sellMenuRef.current && !sellMenuRef.current.contains(event.target as Node)) {
        setIsSellMenuOpen(false);
      }
      if (buyerMenuRef.current && !buyerMenuRef.current.contains(event.target as Node)) {
            ...
            <div className="relative" ref={sellMenuRef}>
              ...
              {isSellMenuOpen && (
                <div className="absolute -right-2 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50">
                  <Link href="/sell/new" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">New Listing</Link>
                  <Link href="/sell/current" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Current Listings</Link>
                  <Link href="/sell/sales" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Sales</Link>
                </div>
              )}
            </div>
